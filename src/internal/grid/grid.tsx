...
export default function Grid({ layout, children, columns, rows }: GridProps) {
  ...
  const cellWidthRef = useRef(((gridWidth || 0) - (columns - 1) * GRID_GAP) / columns);
  useLayoutEffect(() => {
    cellWidthRef.current = ((gridWidth || 0) - (columns - 1) * GRID_GAP) / columns;
  }, [gridWidth, columns]);

  const getWidth = (colspan: number) => colspan * cellWidthRef.current + (colspan - 1) * GRID_GAP;
