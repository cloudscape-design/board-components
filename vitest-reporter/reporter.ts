...
import { AllureConfig, AllureGroup, AllureRuntime, ExecutableItem, LabelName, Stage, Status } from "allure-js-commons";

import { File, Reporter, Task } from "vitest";
import { AllureMeta, AllureStep } from "./interfaces";
  ...
  handleStep(step: AllureStep): ExecutableItem {
    return {
      ...
      attachments: step.attachments.map((attachment) => {
        ...
        return { name: attachment.name, type: attachment.contentType, source: filename };
      }),
