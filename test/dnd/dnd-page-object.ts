...
export class DndPageObject extends ScreenshotPageObject {
  async getGrid() {
    ...
    const placeholderIds = allIds.filter((id) => id.startsWith("awsui"));
    const placehodlerRects = await Promise.all(
      placeholderIds.map((id) => this.getBoundingBox(dashboardWrapper.findItemById(id).toSelector()))
    );
    ...
    const columns = 4;
    const rows = Math.floor(placeholderIds.length / columns);
