...
// items state and reacts only to its own drag-and-drop interactions.
function DemoBoard({
  boardLabel,
  initialItems,
}: {
...
// sibling pattern, shown here living alongside other boards on the same page.
    ...
    <SpaceBetween size="l">
      <Board
        ...
        onItemsChange={({ detail: { items, addedItem, removedItem } }) => {
          setBoardItems(items);
          if (addedItem) {
            setPaletteItems((prev) => prev.filter((item) => item.id !== addedItem.id));
          }
          if (removedItem) {
            setPaletteItems((prev) => [...prev, removedItem].sort((a, b) => a.data.title.localeCompare(b.data.title)));
          }
        }}
        renderItem={(item, actions) => (
          <BoardItem
      ...
      <PageLayout header={<Header variant="h1">Multiple boards on the same page</Header>}>
        <SpaceBetween size="xxl">
          <div>
            ...
            <DemoBoard boardLabel="Board 1" initialItems={boardOneItems} />
          </div>

          <div>
            <Header variant="h2">Board 2 (independent)</Header>
            <DemoBoard boardLabel="Board 2" initialItems={boardTwoItems} />
          </div>

          <div>
            <Header variant="h2">Board 3 with a palette</Header>
            <BoardWithPalette initialBoardItems={paletteBoardItems} initialPaletteItems={paletteItems} />
          </div>
        </SpaceBetween>
      </PageLayout>
    </ScreenshotArea>
