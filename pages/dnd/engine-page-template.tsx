...
export function EnginePageTemplate({
  ...
  const onItemsChange = useCallback(
    ({ detail: { items, addedItem, removedItem } }: CustomEvent<DashboardLayoutProps.ItemsChangeDetail<ItemData>>) => {
      setItems(items);
      if (addedItem) {
  ...
  const renderDashboardItem = useCallback(
    (item: DashboardLayoutProps.Item<ItemData>, actions: DashboardLayoutProps.ItemActions) => (
      <DashboardItem
  ...
  const renderPaletteItem = useCallback(
    (item: PaletteProps.Item<ItemData>) => {
    ...
    <PageLayout header={<Header variant="h1">Configurable dashboard demo</Header>}>
      <div className={classnames[`layout-${layout}`]}>
        ...
        <div className={classnames.palette}>
          ...
          <DashboardPalette items={paletteItems} renderItem={renderPaletteItem} i18nStrings={{}} />
        </div>
      </div>
    </PageLayout>
  );
